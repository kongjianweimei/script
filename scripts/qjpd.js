...
const qjpurlArr = [], qjphddArr = [], qjpbodyArr = [], qjpcount = ''
let qjpurl = $.getdata('qjpurl')
let qjphdd = $.getdata('qjphdd')
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        for (let i = 0; i < qjphddArr.length; i++) {
            if (qjphddArr[i]) {
...
function qjpck() {
    if ($request.url.indexOf("collectPigMoney") > -1) {
        ...
        const qjphdd = JSON.stringify($request.headers)
        if (qjphdd) $.setdata(qjphdd, `qjphdd${status}`)
        $.log(qjphdd)
...
function qjpzp(timeout = 0) {
    return new Promise((resolve) => {
        //token = qjphdd.match(/"Auth-Token":(\d.+)/)[1]
        token = qjphdd.match(/"Auth-Token":"(\w.{35})"/)[1]
        let url = {
